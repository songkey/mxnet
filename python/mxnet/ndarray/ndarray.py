...
def _new_from_shared_mem(shared_handle, shape, dtype):
    hdl = NDArrayHandle()
...
fixed-size items.
    ...
    def _to_shared_mem(self):
        ...
        check_call(_LIB.MXNDArrayCopyToSharedMem(self.handle, ctypes.byref(filename)))
        return py_str(filename.value), self.shape, self.dtype
